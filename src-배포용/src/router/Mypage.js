...
function Mypage() {
    ...
    useEffect(() => {
        const fetchUserInfo = async () => {
            ...
            try {
                const response = await axios.get('http://54.82.4.76:3000/mypage', {
                    headers: {
    ...
    const handleUseMileage = async () => {
        ...
        try {
            const response = await axios.post('http://54.82.4.76:3000/mileage/use', {amount: mileageAmount}, {
                headers: {
                    'Authorization': `Bearer ${token}`
                }
            });
            if (response.status === 200) {
                ...
                const userInfoResponse = await axios.get('http://54.82.4.76:3000/mypage', {
                    headers: {
                        'Authorization': `Bearer ${token}`
                    }
                });
