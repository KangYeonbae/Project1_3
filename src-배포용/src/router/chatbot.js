...
function Chatbot(props) {
    ...
    useEffect(() => {
        ...
        const loadAllData = async () => {
            const newMarkData = await fetchData('http://54.82.4.76:3001/recyclingcenters');
            const zeroData = await fetchData('http://54.82.4.76:3001/zero');
            const napronData = await fetchData('http://54.82.4.76:3001/napron');
    ...
    const handleFileUpload = async (event) => {
        ...
        try {
            const response = await axios.post('http://54.82.4.76:5000/img/upload', formData, {
                headers: {
