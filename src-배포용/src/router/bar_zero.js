...
function ZeroCenters(props) {
    ...
    useEffect(() => {
        const fetchImages = async () => {
            setLoading(true);
            try {
                const response = await axios.get(`http://54.82.4.76:5000/images/${props.selectZeroshop.ID}`);
    ...
    useEffect(() => {
        const fetchRecommendations = async () => {
            ...
            try {
                const response = await axios.get(`http://54.82.4.76:5000/api/recommend/${props.selectZeroshop.ID}`);
