...
def get_city_data():
    try:
        # SHP 파일 로드
        kor = gpd.read_file('build/csv/seoul_EPSG5179.shp', encoding='utf-8')
        ...
        # JSON 형태로 변환된 GeoJSON을 반환
        return jsonify(json.loads(kor_geojson))
    except Exception as e:
        print(f"An error occurred: {e}")
        return jsonify({"error": "Error processing the SHP file"}), 500


if __name__ == '__main__':
    print("Starting Flask server on http://54.82.4.76:5000")
